function [rho,XT1d_rho_max] = disp_nModes_xt_CMT2d_DEN_step_elp(rho,num_phi,step,method,xtmethod,nModes,nPols,pc,KuvSurf,theta_modes,num_theta,statmethod,elp,polrotmethod,deltacore)
...
XT1d_max = zeros(length(rho),num_phi,nModes*nPols);
...
for kk2 = nStart:num_phi
    if mod(kk2,1000) == 0
        if kk2 > 1
            tt = toc(h); h = tic; fprintf([num2str(kk2),' out of ',num2str(num_phi),' - time to finish =',num2str(tt*(num_phi-kk2)/60/1000),' min \n'])
        end
    end
